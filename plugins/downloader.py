...
async def checker(client: Client, message):
    ...
    if [x for x in BLACKLIST if x in message.text]:
        ...
        await message.reply_text("Please Don't Abuse This Service !!")
    else:
        msg = await message.reply_text("Working on it ...", True)
        await msg.edit(text='Choose prefered settings from below 👇', reply_markup=HOME)
...
async def cb_(client: Client, callback_query: CallbackQuery, retry=False):
    ...
    if cb_data == "render":
        ...
        if 'hide' in options_to_change:
            ...
            msg.reply_markup.inline_keyboard.insert(
                -2,
                [InlineKeyboardButton(text='📊 Site Statitics 📊', callback_data="statics")],
            )
            if 'PDF' in msg.reply_markup.inline_keyboard[0][0].text:
                index_to_change = 2
            else:
